...
## Preface
I listen to a lot of metal. If you happen to be uncomfortable with religious or morbid themes appearing in band, album, or track names, I would not recommend reading this post. Albeit, the references to such themes in this post are rather tame.
...
It seems that all rows with `endplay` have `True` for the `skipped` field. The following snippet returns 8,258 rows with `True` and no rows with `False`. Therefore, we will not consider those as full streams.
...
I don't like these column names, so I will simplify them. The `"master_"` prefix may be helpful for tracks that feature more than one artist, but the shorter names are easier to work with.
...
## 4. Retrieving track genres and album art (with Spotipy and the Spotify Web API)

We will create two new columns in our dataframe. 
* `genre`: contains the first listed genre for a given track
* `album_art_url`: contains the url to the smallest size available image of the track's album cover. 
...
Notice that there is no mention of a genre in the dictionary for this track. It turns out that genres are associated with the artist and not a given track. Which makes sense and should work for most cases, as I imagine the number of albums with disparate genres between tracks is quite low. 
...
We have a limited number of queries we can make in a 24‑hour period, so use them wisely. Ideas to reduce requests when searching for artist genres:

1. Get the set of all artists in the DataFrame.
2. Iterate through the artist set.
3. Find the first track for the artist.
4. Retrieve the artist's genres using that track's URI.
5. Assign the retrieved genre to all rows for that artist.
6. Repeat for the next artist.

If the data I received from Spotify came with the artist IDs we could have a much simpler approach of searching for genres with the artist ID instead. It does not seem that the artist IDs are embeded within the Track URI at all. 

### Fetching the genres of my top 35 artists 


```python
top15_artists_listens = df.artist_name.value_counts()[0:15] 
top15_artists_list = list(top15_artists_listens.nlargest(15).index)
top15_artists_list 
```
...
Return the genres of the top 15 artists and populate the `df["genre"]` column with their genres.
...
Let's try getting the genres for the next 20 most common artists and then populate the DataFrame rows for those artists.
...
Let's find out what percentage of my streams are accounted for by my top $x$ artists (with $x$ in $\[1,720\]$ ).
...
So ≈ 70% of my listens are from my top 50 artists. 
...
Only issue now is that I accidentally overwrote some earlier genre dictionaries, so I no longer have those intermediate files.
...
### Fetching the album art for my top 10 artists
We'll only do this for the top 10–20 artists. The others will have NaN or `None` as their entry for `album_art_url`. 


```python
def get_album_art(sp, track_uri): 
    track = sp.track(track_uri)
    album_art_url = track['album']["images"][-2]["url"] # 2nd to smallest size
...
def mass_album_art_fetch(sp, album_list,df): 
    ...
    for x in album_list : 
        row_0 = next(df.itertuples(index=False))
...
Some of these genres don't seem like things I would listen to. While it is possible, I suspect some genre assignments (such as "german pop") are incorrect for certain artists.

The band Greta has been categorized as "german pop".  Listen to the first 30 seconds of [this song](https://open.spotify.com/track/53M6HombmleUN6AF0Ptmgg?si=7e962a09ea054051) and tell me if this band sounds like they make german pop. 

>Spoiler: They do not. They are a 90's grunge band. 
{:.notice--primary}

Greta appears to be a Spotify profile that hosts music from different artists with the same name; this likely produced the incorrect "german pop" genre for the Greta I listen to. I replaced that genre with "grunge".
...
Let's get album-art URLs for the top 10 (or possibly 20) albums before fetching any more genres. According to the Spotipy documentation, each album has cover-art URLs in multiple sizes; the images are ordered from largest to smallest, so higher indices correspond to smaller images.
...
Another logarithmic curve, we only need to retrieve a fraction of the top albums' cover art in order to retrieve the album art for most of my streams. The main motivation for fetching album art is to have some visuals for our dashboard. So I am satisfied with only fetching the cover art for the first top 20 albums. 
...
> While working on the Tableau dashboard, I realized that one of our top three songs didn't have a URL for its album art. So I fetched it manually.  
{:.notice--primary}
...
It is evident from the visuals that I started the year with my listening being highly concentrated in rap , with that share of my listening being reduced dramatically over the months as metal subgenres began to dominate my listening habits.
